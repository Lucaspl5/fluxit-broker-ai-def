...
export class BacktestingService {
  ...
  async runBacktest(params: BacktestParams): Promise<string> {
    ...
    const allBars = await this.alpaca.getHistoricalData(symbol, '1Day', 500);
    ...
    // Walk forward — need at least 200 bars of history before the range
    ...
    for (let i = 0; i < barsInRange.length; i++) {
      ...
      if (!position) {
        ...
        if (convergence?.type === 'BUY') {
          ...
          const qty        = Math.max(1, Math.floor(riskAmount / currentPrice));
  ...
  private calcSharpe(trades: BacktestTrade[]): number | null {
    ...
    // Annualized Sharpe (approx 252 trading days, scaled by trade count)
    return (mean / stdDev) * Math.sqrt(252 / trades.length);
  }
